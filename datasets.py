...
class MpMPiigaze(Dataset):
  def __init__(self, pathorg, root, transform, train, angle,fold=0):
    ...
    if train==True:
      ...
      path=path[fold]
    if isinstance(path, list): #path가 리스트인지 아닌지를 판단하며 return 값은 true 또는 false입니다.
        for i in path:
            with open(i) as f:
                lines = f.readlines()   #readlines() 메서드는 파일의 모든 행을 읽고 list로 변수에 저장합니다. 각 행은 하나의 요소로 저장되지만 큰 파일을 읽으면 메모리를 차지합니다.
                lines.pop(0) #첫 번째 원소를 삭제합니다.
                self.orig_list_len += len(lines)
                for line in lines:
  ...
  def __getitem__(self, idx):
    ...
    labels = binned_pose
    cont_labels = torch.FloatTensor([pitch, yaw])  #list, numpy를 tensor로 변환합니다.
