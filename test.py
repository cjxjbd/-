...
def parse_args():
    ...
    parser.add_argument(
        ...
        default='datasets/MPIIFaceGaze/test', type=str)
    # Important args -------------------------------------------------------------------------------------------------------
    # ----------------------------------------------------------------------------------------------------------------------
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default='models/_epoch_50', type=str)
    parser.add_argument(
    ...
    parser.add_argument(
        '--bins', dest='bins', help='bins',
        default=28, type=int)
    parser.add_argument(
        '--angle', dest='angle', help='angle',
        default=10, type=int)

    parser.add_argument(
        '--bin_width', dest='bin_width', help='bin_width',
        default=6, type=int)
...
if __name__ == '__main__':
    ...
    if data_set=="gaze360":
        ...
        for fold in range(1):
            ...
            folder=list(filter(lambda f: f[-4:] in ['.pkl','.tar'],folder))
            folder.sort(key=natural_keys)
            ...
            with open(os.path.join(evalpath, os.path.join("fold"+str(fold), data_set+".log")), 'w') as outfile:
                ...
                for epochs in folder: 
                    ...
                    #saved_state_dict = torch.load(
                     #   os.path.join(snapshot_path + "/fold" + str(fold), '_epoch_50.pth.tar'))
                    ...
                    with torch.no_grad():
                        for j, (images, labels, cont_labels, name) in enumerate(test_loader):
                            ...
                            pitch_predicted = pitch_predicted*np.pi/180   #각도를 라디안로 변환합니다.
                            yaw_predicted = yaw_predicted*np.pi/180
